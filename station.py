...
class MixingStation:
    ...
    def _draw_hud(self, screen):
        rect = self.hud_rect
        self._panel(screen, rect, self.CYAN, self.PANEL, radius=12, width=2)
        ...
        if self.progression is not None:
            level = getattr(self.progression, "level", level)
            xp = getattr(self.progression, "xp", xp)
        if self.economy is not None:
            player_name = getattr(self.economy, "player_name", player_name)
            credits = getattr(self.economy, "credits", credits)
            if hasattr(self.economy, "get_credits"):
                try:
                    credits = self.economy.get_credits()
                except Exception:
                    pass
            location = getattr(self.economy, "location", location)
        if self.rewards is not None:
            combo = getattr(self.rewards, "combo", combo)
        if not player_name:
            player_name = "PLAYER"
        if not location:
            location = "UNKNOWN"
        ...
        max_xp = 100
        if self.progression is not None:
            xp_requirements = getattr(self.progression, "xp_requirements", None)
            if isinstance(xp_requirements, dict):
                max_xp = xp_requirements.get(level, 100)
            else:
                possible = getattr(self.progression, "xp_required", None)
        ...
        name_text = self.font_hud.render(str(player_name)[:14].upper(), True, self.WHITE)
        screen.blit(name_text, (35, 20))

        level_text = self.font_small.render(f"LEVEL {level}", True, self.CYAN_LIGHT)
        screen.blit(level_text, (35, 40))

        xp_label = self.font_small.render("XP", True, self.PINK_LIGHT)
        screen.blit(xp_label, (150, 22))

        xp_bar = pygame.Rect(174, 20, 225, 18)
        pygame.draw.rect(screen, (18, 25, 48), xp_bar, border_radius=9)
        pygame.draw.rect(screen, self.CYAN, xp_bar, width=1, border_radius=9)
        ratio = max(0.0, min(1.0, xp / max_xp))
        fill_width = int((xp_bar.width - 6) * ratio)
        if fill_width > 0:
            pygame.draw.rect(
                screen,
                self.CYAN,
                pygame.Rect(xp_bar.x + 3, xp_bar.y + 3, fill_width, xp_bar.height - 6),
                border_radius=6,
            )
        xp_number = self.font_small.render(f"{xp}/{max_xp}", True, self.WHITE)
        screen.blit(xp_number, xp_number.get_rect(center=xp_bar.center))

        credit_text = self.font_hud.render(f"CREDITS ${credits}", True, self.YELLOW)
        screen.blit(credit_text, (420, 20))

        location_text = self.font_small.render(f"LOCATION: {str(location).upper()}", True, self.SOFT_WHITE)
        screen.blit(location_text, (420, 40))

        combo_text = self.font_small.render(f"COMBO x{combo}", True, self.PINK_LIGHT)
        screen.blit(combo_text, (650, 40))

        if time.monotonic() < self.reward_feedback_until:
            feedback_parts = []
            if self.last_xp_change > 0:
                feedback_parts.append(f"+{self.last_xp_change} XP")
            elif self.last_xp_change < 0:
                feedback_parts.append(f"{self.last_xp_change} XP")
            if self.last_credit_change > 0:
                feedback_parts.append(f"+${self.last_credit_change}")
            elif self.last_credit_change < 0:
                feedback_parts.append(f"-${abs(self.last_credit_change)}")
            if feedback_parts:
                feedback = self.font_small.render(
                    "  ".join(feedback_parts),
                    True,
                    self.GREEN if (self.last_xp_change >= 0 and self.last_credit_change >= 0) else self.PINK_LIGHT,
                )
                screen.blit(feedback, (600, 20))
